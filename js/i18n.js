...
(function () {
  ...
  function initSwitcher() {
    ...
    // Guard: if this ever runs twice on the same markup (e.g. a partial/
    // AJAX page-loader re-executes this script, or a duplicate <script>
    // tag ends up on the page) skip re-binding so clicks don't fire
    // multiple times or fight each other.
    if (wrap.getAttribute('data-i18n-bound') === '1') return;
    wrap.setAttribute('data-i18n-bound', '1');
  ...
  function boot() {
    try {
      ...
      // Never let a translation/init error leave the switcher unclickable.
      if (window.console && console.error) console.error('LARAS i18n: initSwitcher failed', e);
    }
    try {
      applyStaticTranslations(currentLang()); // no fade on first load (hidden behind loader)
    } catch (e) {
      if (window.console && console.error) console.error('LARAS i18n: applyStaticTranslations failed', e);
    }
  }

  // Run as soon as the DOM is ready. If this script executes after
  // DOMContentLoaded has already fired (e.g. the header markup was
  // injected later via a partial/AJAX loader — common in CI3 setups),
  // the event would never fire again and the switcher would stay dead.
  // Falling back to readyState covers that case.
